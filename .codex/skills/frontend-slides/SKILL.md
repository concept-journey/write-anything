...
description: Use when the user wants to create, convert, improve, share, or export an HTML presentation, slide deck, talk deck, PowerPoint, PPT, or PPTX, especially when visual style exploration and browser-based slides matter.
---
...
### Korean / CJK Line Break Rules

When a deck contains Korean, Japanese, or Chinese text, line breaking is a design requirement, not a browser default.

- Add CJK-safe wrapping to readable text blocks: `word-break: keep-all; overflow-wrap: break-word; line-break: strict;`.
- Use `text-wrap: balance` for large headings and statement slides when supported; use `text-wrap: pretty` for paragraph-like copy.
- For large Korean headlines, manually choose `<br>` breakpoints at semantic phrase boundaries instead of letting the browser split awkwardly after particles, endings, or short modifiers.
- After adding manual breaks, re-check every slide for overflow at the fixed 1920×1080 stage size.
- Do not solve Korean wrapping by shrinking text until hierarchy is lost; split the sentence or the slide instead.
